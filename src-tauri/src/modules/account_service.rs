...
impl AccountService {
    ...
    pub async fn prepare_oauth_url(
        ...
        let modules::integration::SystemManager::Desktop(handle) = &self.integration;
        modules::oauth_server::prepare_oauth_url(Some(handle.clone()), oauth_client_key).await
    }
    ...
    pub async fn start_oauth_login(
        ...
        let modules::integration::SystemManager::Desktop(handle) = &self.integration;
        let token_res =
            modules::oauth_server::start_oauth_flow(Some(handle.clone()), oauth_client_key).await?;
        self.process_oauth_token(token_res).await
    }

    pub async fn complete_oauth_login(&self) -> Result<Account, String> {
        let modules::integration::SystemManager::Desktop(handle) = &self.integration;
        let token_res = modules::oauth_server::complete_oauth_flow(Some(handle.clone())).await?;
        self.process_oauth_token(token_res).await
    }
