...
*   **版本演进**:
    *   **v1.1.0 (2026-09-24)**:
        -   **macOS 安装限制**：本版仅提供 Apple Silicon 与 Intel 的 macOS 安装包；应用未使用 Apple Developer ID 签名，也未经 Apple 公证。macOS 可能阻止首次打开，请核对 Release 下载来源和 SHA256SUMS，并参照 [Apple 官方说明](https://support.apple.com/zh-cn/102445) 在系统设置中决定是否“仍要打开”。
        -   **账号凭据安全**：日常界面不再接收账号 Token；账号导入、导出通过原生文件选择，导出文件私有原子写入；OAuth 授权加入 PKCE，日志不再记录完整代理地址。
        -   **主程序接力**：账号卡片只保留 Antigravity 主程序切换；系统凭据或兼容 SQLite 写入后读回核验，失败明确报错并尝试重启客户端。
        -   **数据恢复与配额**：设备身份写入前保留完整备份并原子替换；目录迁移先启用新位置后清理旧目录；配额未知与耗尽分开展示，过期配额不参与接力推荐。
        -   **发布门禁**：缺少任一 macOS 架构产物或 Tauri 更新签名时停止发布，生成 SHA256SUMS，安装脚本核验下载文件；真实账号切换与自动更新仍需目标机器验收。
        -   **语言收敛**：界面与托盘仅保留简体中文、英文；旧繁体中文设置迁移到简体中文，其他已移除语言迁移到英文。
        -   **设置收敛**：移除已下线的 CLI 与 IDE 切换路径设置；保留主程序路径，旧 IDE 路径仅用于后台进程保护。
        -   **更新检查设置**：开关与间隔合并为同一设置区，间隔输入校验 1–168 小时；保存时保留上次检查时间，成功完成版本检查后才记录检查时间。

    *   **v1.0.1 (2026-09-23)**:
        -   **AMT 独立项目首个公开版本**：聚焦 Antigravity 多账号管理、真实配额、推荐接力、设备身份和原生客户端切换；移除旧版网关与代理能力。
        -   **账号体验**：简化核心配额展示，改进添加账号与导入流程，并调整账号卡片布局、展开行为及操作反馈。
        -   **发布基础**：安装入口、应用更新地址和 Release 工作流统一指向本仓库。
        -   **版本迁移提示**：此前安装的 4.7.x 开发构建需手动安装本版；自动更新不会把较高版本号识别为 1.0.1 的旧版本。
    *   **v4.7.12 (2026-09-21)**:
